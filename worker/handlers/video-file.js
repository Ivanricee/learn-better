...
/**
 * Handler para tipo: video (archivo subido)
 * Pipeline: ffmpeg → OGG → validar duración → chunking → Whisper → vectorizar
 *
 * @param {object} params
 * @param {string} params.jobId
 * @param {string} params.fileId
 * @param {string} params.categoryId
 * @param {string} params.storagePath - Ruta local del video subido
...
export async function processVideoFile({
  ...
  try {
    // 1. Extraer audio con ffmpeg → OGG 24kbps mono
    ...
    // 6. Done
    // Nota: generateTemario → fase futura
    await updateJob(jobId, {
      status: "done",
      step: "done",
      progress: 100,
